...
float openpbr_bsdf_total_pdf(in LobeProbabilities probs, in LobePDFs pdfs)
{
                         ...
                         out vec3 woutputL, out float pdf_woutputL, inout int rndSeed)
{
    ...
    for (int lobe_id=0; lobe_id<NUM_LOBES; ++lobe_id)
    {
        ...
        if (X < CDF || lobe_id==NUM_LOBES-1)
        {
            ...
            LobePDFs pdfs;
            openpbr_bsdf_lobe_pdfs(pW, basis, winputL, woutputL, pdfs);

            // Thus compute the PDF of the total BRDF
            pdf_woutputL = openpbr_bsdf_total_pdf(probs, pdfs);
                       ...
                       inout int rndSeed)
{
    ...
    LobePDFs pdfs;
    openpbr_bsdf_lobe_pdfs(pW, basis, winputL, woutputL, pdfs);
    return openpbr_bsdf_total_pdf(probs, pdfs);
}
